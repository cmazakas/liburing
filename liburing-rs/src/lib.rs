...
pub unsafe extern "C" fn io_uring_prep_splice(
    ...
    io_uring_prep_rw(io_uring_op_IORING_OP_SPLICE as _, sqe, fd_out, ptr::null_mut(), nbytes, off_out as u64);
...
pub unsafe extern "C" fn io_uring_prep_readv_fixed(
    ...
    (*sqe).opcode = io_uring_op_IORING_OP_READV_FIXED as _;
    (*sqe).__liburing_anon_4.buf_index = buf_index as u16;
}
...
pub unsafe extern "C" fn io_uring_recvmsg_cmsg_nexthdr(
    ...
    let cmsg = cmsg.cast::<u8>().add(CMSG_ALIGN((*cmsg).cmsg_len)).cast::<cmsghdr>();
...
unsafe fn __io_uring_peek_cqe(
    ring: *mut io_uring, cqe_ptr: *mut *mut io_uring_cqe, nr_available: *mut c_uint,
) -> c_int {
    let mut cqe;
    let mut err = 0;
...
pub unsafe extern "C" fn io_uring_buf_ring_advance(br: *mut io_uring_buf_ring, count: c_int) {
    ...
    io_uring_smp_store_release(&mut (*br).__liburing_anon_1.__liburing_anon_1.as_mut().tail, new_tail);
}
