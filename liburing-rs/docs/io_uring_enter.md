...
**IORING_SETUP_R_DISABLED** via **io_uring_register**(2) or
...
wait regions previously registered with **io_uring_register**(2) using
...
not specified in the call to **io_uring_setup**(2)), an application may
...
is not NULL, **io_uring_enter**(2) first replaces the current signal
...
**pwritev2**(2). If the file is not seekable, *off* must be set to zero
or -1.
...
File sync. See also **fsync**(2). Optionally *off* and *len* can be used
...
**epoll_ctl**(2) for details of the system call. *fd* holds the file
...
See also **sync_file_range**(2) for the general description of the
...
system call. See also **sendmsg**(2) for the general description of the
...
If set, io_uring will assume the socket
...
also **accept4**(2) for the general description of the related system
...
**IORING_OP_CONNECT**\
Issue the equivalent of a **connect**(2) system call. *fd* must be set
...
must contain the length. See also **fallocate**(2) for the general
...
**posix_fadvise**(2) for the general description of the related system
...
**openat2**(2) for the general description of the related system call.
Available since 5.6.
...
filled in. See also **statx**(2) for the general description of the
...
**read**(2) and **write**(2) system calls. These are non-vectored
versions of the **IORING_OP_READV** and **IORING_OP_WRITEV** opcodes.
See also **read**(2) and **write**(2) for the general description of the
related system call. Available since 5.6.
...
of the file descriptors must refer to a pipe. See also **splice**(2) for
the general description of the related system call. Available since 5.7.

<!-- -->

**IORING_OP_TEE**\
Issue the equivalent of a **tee**(2) system call. *splice_fd_in* is the
...
**tee**(2) for the general description of the related system call.
Available since 5.8.
...
**IORING_OP_SHUTDOWN**\
Issue the equivalent of a **shutdown**(2) system call. *fd* is the file
...
**IORING_OP_RENAMEAT**\
Issue the equivalent of a **renameat2**(2) system call. *fd* should be
...
should be set to the *flags* passed in to **renameat2**(2). Available
since 5.11.

<!-- -->

**IORING_OP_UNLINKAT**\
Issue the equivalent of a **unlinkat**(2) system call. *fd* should be
set to the *dirfd*, *addr* should be set to the *pathname*, and
*unlink_flags* should be set to the *flags* being passed in to
**unlinkat**(2). Available since 5.11.

<!-- -->

**IORING_OP_MKDIRAT**\
Issue the equivalent of a **mkdirat**(2) system call. *fd* should be set
to the *dirfd*, *addr* should be set to the *pathname*, and *len* should
be set to the *mode* being passed in to **mkdirat**(2). Available since
5.15.

<!-- -->

**IORING_OP_SYMLINKAT**\
Issue the equivalent of a **symlinkat**(2) system call. *fd* should be
set to the *newdirfd*, *addr* should be set to the *target* and *addr2*
should be set to the *linkpath* being passed in to **symlinkat**(2).
Available since 5.15.

<!-- -->

**IORING_OP_LINKAT**\
Issue the equivalent of a **linkat**(2) system call. *fd* should be set
...
**IORING_OP_SOCKET**\
Issue the equivalent of a **socket**(2) system call. *fd* must contain
...
must be set to zero. See also **socket**(2) for the general description
...
**ioctl**(2). Further information may be found in the dedicated man page
...
*addr_len* specifying its size, turning the request into a **sendto**(2)
...
If set, instructs io_uring to use a
pre-mapped buffer. The *buf_index* field should contain an index into an
array of fixed buffers. See **io_uring_register**(2) for details on how
to setup a context for fixed buffer I/O.

**IORING_OP_SENDMSG_ZC**\

Issue the zerocopy equivalent of a **sendmsg**(2) system call. Works
...
Issue the equivalent of a **waitid**(2) system call. *len* must contain
...
being filled in. See also **waitid**(2) for the general description of
...
**fsetxattr**(2) or **fgetxattr**(2) system call. *addr* must contain a
...
**setxattr**(2) or **getxattr**(2) *addr3* must contain a pointer to the
path of the file. For **fsetxattr**(2) or **fgetxattr**(2) *fd* must
...
Issues the equivalent of the **bind**(2) system call. *fd* must contain
...
Issues the equivalent of the **listen**(2) system call. *fd* must
...
Issues the equivalent of the **ftruncate**(2) system call. *fd* must
...
Issues the equivalent of the **futex_wait**(2) system call. *addr* must
hold a pointer to the futex, *addr2* must hold the value to which the
futex has to be changed so this caller to **futex_wait**(2) can be woken
by a call to **futex_wake**(2), *addr3* must hold the bitmask of this
**futex_wait**(2) caller. For a caller of **futex_wake**(2) to wake a
...
Issues the equivalent of the **futex_wake**(2) system call. *addr* must
...
this **futex_wake**(2) call. To wake a waiter additionally the bitmask
...
Issues the equivalent of the **futex_waitv**(2) system call. *addr* must
...
registered via **io_uring_register**(2) using
...
**epoll_wait**(2). *fd* must be set to the epoll file descriptor, *addr*
...
section of the **io_uring_register**(2) man page). Note that this isn't
...
the queue tail is advanced, the **io_uring_enter**(2) system call can be
...
through this field. They are described in section [CQE ERRORS](#cqe-errors).

For read and write opcodes, the return values match *errno* values
documented in the **preadv2**(2) and **pwritev2**(2) man pages, with
...
**io_uring_enter**(2) returns the number of I/Os successfully consumed.
...
a completion queue entry (see section [CQE ERRORS](#cqe-errors)), rather than
...
**io_uring_enter**(2) will not have this error. This error should be
...
due to registered restrictions. See **io_uring_register**(2) for details
