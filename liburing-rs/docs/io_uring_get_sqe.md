...
The [io_uring_get_sqe] function gets the next available submission
queue entry from the submission queue belonging to the *ring* param.

On success [io_uring_get_sqe] returns a pointer to the submission
queue entry. On failure NULL is returned.

If a submission queue entry is returned, it should be filled out via one
of the prep functions such as [io_uring_prep_read] and submitted
via [io_uring_submit].
...
[io_uring_get_sqe] returns a pointer to the next submission queue
