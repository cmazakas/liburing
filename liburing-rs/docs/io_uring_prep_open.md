...
The [io_uring_prep_openat] function prepares an openat request. The
submission queue entry *sqe* is setup to use the directory file
descriptor *dfd* to start opening a file described by *path* and using
the open flags in *flags* and using the file mode bits specified in
*mode*. Similarly [io_uring_prep_open] prepares an open request.
...
appropriate size. Once registered, a direct accept request may use any
entry in that table and is specified in *file_index* , as long as it is
within the size of the registered table. If a specified entry already
